package com.example.numpang.ui.screen
...
class AuthViewModel (
    ...
    fun signup(email: String, password: String, name: String, nim: String, phone: String, photoUrl: String) {
        viewModelScope.launch {
            if (email.isEmpty() || password.isEmpty()) {
                _authState.value = AuthState.Error("Email or password can't be empty")
                return@launch
            ...
            _authState.value = AuthState.Loading
            val result = repository.signUp(email, password, name, nim, phone, photoUrl)
            result.onSuccess { user ->
                val roleResult = repository.getUserRole(user.uid)
                roleResult.onSuccess { role ->
                    _authState.value = AuthState.Authenticated(role)
